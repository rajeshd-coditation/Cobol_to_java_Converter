...
- [ ] **Phase 3 finisher: extract the compileAndRun + repair loop.** Phases 3a/3b/3d landed. `processFile` was 670 lines with deep closure captures; four pure helpers (preflightCheck, buildContext, extractReviewerFeedback, extractEntrySignature) moved to `src/routes/convert-azure-helpers.js` in 3b, each unit-tested. `processFile` itself is now ~480 lines with the orchestration + compile/repair/write/accuracy steps still inline. The `compileAndRun` closure at its center is the next candidate — it captures `javaDir / workDir / javaClassName / javaFileName` from outer scope. Extracting needs passing those as args. Low priority. After this lands, processFile is mostly just the stage-ordering loop.
